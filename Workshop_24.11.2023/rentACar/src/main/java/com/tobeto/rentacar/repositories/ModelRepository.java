...
public interface ModelRepository extends JpaRepository<Model, Integer> {
    ...
    @Query("SELECT new com.tobeto.rentacar.services.dtos.model.responses.GetListModelResponse(m.id,m.name,m.brand) " +
            "FROM Model m WHERE LOWER(m.name) LIKE LOWER(CONCAT(:name, '%'))")
    ...
    @Query("SELECT new com.tobeto.rentacar.services.dtos.model.responses.GetListModelNameByBrandResponse(m.id,m.name) " +
            "FROM  Model m WHERE LOWER(m.brand.name) = LOWER(:brandName)")
