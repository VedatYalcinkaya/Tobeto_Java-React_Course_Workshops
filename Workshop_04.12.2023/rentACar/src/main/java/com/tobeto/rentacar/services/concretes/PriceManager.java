...
public class PriceManager implements PriceService {
    ...
    public List<GetListPriceResponse> getAll() {
        return priceRepository.findAll().stream().map(price -> new GetListPriceResponse(price.getId(), price.getDailyPrice(), price.getWeeklyPrice(), price.getMonthlyPrice())).toList();
    }
