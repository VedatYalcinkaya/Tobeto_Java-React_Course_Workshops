...
public class Main {
    public static void main(String[] args) {
        ...
        List<GetListBrandResponse> result = brands.stream().map((brand) -> new GetListBrandResponse(brand.getId(),brand.getName())).toList();

        List<Brand> filterBrands = brands.stream().filter((brand) -> brand.getName().length() > 4).toList();
        System.out.println(filterBrands.size());
