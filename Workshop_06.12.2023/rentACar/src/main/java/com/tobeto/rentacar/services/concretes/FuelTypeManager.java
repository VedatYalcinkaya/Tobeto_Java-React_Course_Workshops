...
public class FuelTypeManager implements FuelTypeService {
    ...
    public void add(AddFuelTypeRequest request) {
        //Validation ile halledilemeyecek bir iş kuralı bulamadım :S
    ...
    public List<GetListFuelTypeResponse> getAll() {
        return fuelTypeRepository.findAll().stream().map(fuelType -> new GetListFuelTypeResponse(fuelType.getId(), fuelType.getName())).toList();
    }
